...
public class RegExp03Main {
	...
	public static void main(String[] args) {
		...
		arrInput = new String[] {
				"The Things",
				"On The Things", // 매칭 없다
				" The The The", // 공백도 안됨 (매칭 없다)
				"The The the The" // 한개만 매칭! (시작하는 문자열) 매치1: The {0~3}
		...
		arrInput = new String[] {
				...
				"xbz",				
				"xyyz x1y xyyx xxxxyxxxyxxxyyyxyxyxyxy"
		};
		
		//─────────────────────────────────────────
		title = " * : 바로 앞의 문자가 없거나 한개 이상의 경우를 매칭";
		regex = "ab*"; // a로 시작하고, b는 없거나 여러개 가능!
		arrInput = new String[] {
				"a",
				"abc",
				"ab",
		...
		regex = "ab?"; // a로 시작하고 b는 있거나 하나만 있거나~
		arrInput = new String[] {
				"a", //1
				"abc", //1
				"kkabcc", //1
		...
		regex = "[abc]+";
		regex = "[a-z]+"; // a ~ z 까지 1개 이상의 문자열 // 소문자로 이루어진 단어로 매칭되는 정규표현식 **
		arrInput = new String[] {
				"abc100",
		...
		arrInput = new String[] {
			...
			"-10",
			"012"
		}; // 자연수 1번째 자리는 1~9 / 2번째 자리부터는 0~9 없아도 된다.
		regex = "^[1-9][0-9]*$"; // 자연수에 대한 정규표현식
		
		//─────────────────────────────────────────
		title = " {} : 앞에 있는 문자나 문자열의 등장개수를 정함";
		regex = "ab{2}"; // a로는 반드시 시작되고, b가 2개 매칭되는것
		arrInput = new String[] {
				"abb", //1
				"abbb", //1
		...
		regex = "ab{3,5}"; // b의 개수가 3개부터 5개 까지, //띄어쓰기하면 안댐!! (err: PatternSyntaxException)
		
		//─────────────────────────────────────────
		title = " () : ()안에 있는 글자들을 그룹화 ";
		regex = "a(bc)*"; // a로 시작하고, (bc)가 없거나 하나 이상
		arrInput = new String[] {
				"abc",
				"abcbcbbca",
				"abcabcabc",
