...
-- 인덱스가 없으면 테이블에서 특정값을 찾기 위해 모든 데이터를 다 뒤지는 TABLE SCAN 발생 -- 빈번하게 발생하면, 속도 저하 발생!! 
-- 기본키를 위한 인덱스를 기본 인덱스,   
...
-- 사원의 인덱스
SELECT empno, ROWID FROM T_EMP; -- primary key
-- ROWID 는 EMPNO 순서로 설정된다!! 
-- ROWID, ROW INSERT 될때마다 부여되므 SELECT 시 기본적으로 참조 

SELECT empno, ROWID FROM t_emp ORDER BY DESC; -- ROWID도 내임차순이 됨

SELECT ENAME, rowid 
FROM T_EMP; -- rowid 순서로 정렬됨!! 
...
SELECT * FROM T_DEPT2 ;
CREATE UNIQUE INDEX idx_dept2_dname ON t_dept2(dname); -- 설정 된 이후로 중복된 값은 들어갈 수 없다!! 

INSERT INTO T_DEPT2 VALUES (9100, '임시매장' , 1006, '서울지사'); -- OK!

INSERT INTO T_DEPT2 VALUES (9101, '임시매장' , 1006, '부산지사'); -- OK!
-- err : ORA-00001: unique constraint (SCOTT0316.IDX_DEPT2_DNAME) violated
...
-- 오름차순 특화 인덱스, 내림차순 특화 인덱스 등 따로 설정 할 수 있다.
...
CREATE INDEX idx_prof_pay_fbi ON t_professor(pay + 100); -- function based index 
