...
-- date 타입으로 기본적인 연산이 가능하다
-- 기본적인 날짜 연산
SELECT 
	SYSDATE "오늘",
	SYSDATE + 1 "내일(24시간 뒤)", -- 1이 하루!! 
	SYSDATE - 2 "그저께",
	SYSDATE + 1/24 "한시간 뒤" 
FROM DUAL ;
...
SELECT 
	...
	SYSDATE - TO_DATE('2020-03-16 09:00:00', 'YYYY-MM-DD hh:mi:ss') "경과한 날"  
FROM DUAL;

-- MONTHS_BETWEEN : 날짜 사이의 개월수
SELECT 
	-- 규칙1 : 두 날짜중 큰 날짜를 먼저 입력해야, 양수값으로 나온다!!
	MONTHS_BETWEEN('2020-03-01', '2020-01-01') "양수값",
	MONTHS_BETWEEN('2020-01-01', '2020-03-01') "음수값",
	MONTHS_BETWEEN('2020-01-01', '2020-03-12') "음수값",
...
SELECT NAME , TO_CHAR(SYSDATE, 'YYYY-MM-DD') 오늘, 
		...
		TRUNC(MONTHS_BETWEEN(TO_CHAR(SYSDATE, 'YYYY-MM-DD'), TO_CHAR(HIREDATE , 'YYYY-MM-DD')), 1) 근속개월,
...
SELECT 
	...
	NEXT_DAY(SYSDATE, '월' ) "다음 월요일"  -- 특정 요일의 날짜를 표시
FROM dual;
