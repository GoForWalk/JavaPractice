...
public class DB01Main {
	...
	public static void main(String[] args) {
		...
		ResultSet rs = null; // java.sql.ResultSet (약자는 보통 rs 로 고정)
		
		try {
			// 2. JDBC 드라이버 클래스를 메모리에 로드
			Class.forName(DRIVER);// <-- 동적 클래스 로딩 (실행 중에 메모리를 로딩할 수 있다, 원래는 프로그램 켜질때만 켜진다.)
			System.out.println("드라이버 클래스 로딩 성공");
			
			// 3. DB와 connection
			conn = DriverManager.getConnection(URL, USER, PASSWD);
			System.out.println("DB Connect 연결"); // conn 은 자원이기 때문에 사용이 끝나면, 반드시 close 해야한다.
			
			// 4. Statement 인스턴스를 생성 (SQL을 사용하기 위한 인스턴스)
			stmt = conn.createStatement(); // stmt 는 자원이기 때문에 사용 후 close;
			System.out.println("Statement 생성 성공");
			
			// 5. SQL 문장 작성(SELECT, INSERT, UPDATE, DELETE)
			System.out.println();
			String sql_insert = "INSERT INTO test_member VALUES(100, '마징가', SYSDATE)"; // sql 문법으로 작성
...
//			// getXXX()에 '컬럼 명' 혹은 ' 별명' 명시 (만약 별명으로 설정했을시, 반드시 별명으로 "별명" 해야한다. 
...
//				System.out.println(result);
//				}
			// dataBase는 숫자 시작을 1 부터 하는 경우가 많다!!
			// 따라서 첫번째 인덱스가 1부터 시작!!!! 
...
//			while(rs.next()) { // next() 레코드 하나 추출하고 true 리턴, 더이상 뽑아낼 레코드 없으면 false 리턴.
//				
//				int no =	rs.getInt(1); // getXXX()에 인덱스 명시  가능
//				// mb_no 컬럼 값이 0 이면, 0 리턴!! 따라서 null을 걸러낼수 없다!!
//				
//				String name = rs.getString(2);
//				
//				
