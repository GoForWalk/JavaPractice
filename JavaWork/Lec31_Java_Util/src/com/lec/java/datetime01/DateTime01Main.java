...
public class DateTime01Main {
	...
	public static void main(String[] args) throws Exception{
		...
		// 짝대기 ?ㅅ? : deprecated : 앞으로는 이것을 쓰지 않습니다. (지금 동작은 한다.)
		int month = now.getMonth() + 1; // 0 월 부터 시작
		int day = now.getDate();
		int hour = now.getHours();
		int minute = now.getMinutes();
		int second = now.getSeconds();
		int weekDay = now.getDay(); // 요일: 일요일이 0 ~ 시작
		
		System.out.println(year + "년 " + month + "월 " + day + "일    " + hour + "시 " + minute + "분 "+ second +"초 " ); 
		
		System.out.println();
		System.out.println("3. SimpleDateFormat 사용한 출력"); // 이걸 보통 사용한다!!!
		SimpleDateFormat fmt = new SimpleDateFormat("yyyy년 MM월 dd일 hh시 mm분 ss초"); // hh : 12시간제 , HH : 24시간제
		...
		long baseTime = System.currentTimeMillis(); // 현재 시각 ms(millisecond)로 리턴
													// 1970-01-01 00:00:00 UTC 기준으로 경과된 ms
		System.out.println(baseTime);
		long curTime = baseTime + 2000;  // 2초 뒤의 시간 값 ms
		
		SimpleDateFormat mSimpleDateFormat = new SimpleDateFormat("yyyy:MM:dd-hh:mm:ss", Locale.KOREA); // 지역 이름
		...
		System.out.println("결과시간 : " + lapTime + " ms");
