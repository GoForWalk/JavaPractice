...
<%--
	...
	requset.setAttribute("person1", person1); // 좀 더 정확히는 이 코드도 수행한다.
 --%>


	<%-- 기본생성자 생성후 useBean 안에서 setProperty 사용 --%>
	<jsp:useBean id="person2" class="com.lec.beans.Person" scope="page">
		<jsp:setProperty property="name" name="person2" value="명기봄"/><br>
		<jsp:setProperty property="age" name="person2" value="36"/><br>
		<jsp:setProperty property="id" name="person2" value="998877"/><br>
		<jsp:setProperty property="gender" name="person2" value="남"/><br>
	</jsp:useBean>
	
	<%-- form에서 넘어온 값을 bean 객체가 바로 받을 수 있다.
			bean 의 필드명(getter/setter 명) 과 form 의 name이 일치해야 한다. 
	 --%>
	 
	 <jsp:useBean id="person3" class="com.lec.beans.Person" scope="page"/>
		<jsp:setProperty property="name" name="person2"/>
		<jsp:setProperty property="age" name="person2" />
		<jsp:setProperty property="id" name="person2" />
		<jsp:setProperty property="gender" name="person2" /> 
