...

{
    ...
    if(a){ // <-- new 를 사용하여 생성하면 조건식에서 '참' 으로 판정한다.
        console.log(a, '참입니다.');
    } else {
        console.log(a, '거짓입니다.');
    }
    ...
    const b = Boolean(false); // Boolea() 함수 --> 함수가 객체 역할을 한다.
...
console.log('\n<number>')
{
    ...
    d = Number('Mark'); // number 로 형변환 됨! 하지만 Number type으로 변환이 되지 않는다!
    ...
    d = parseInt('3.14'); // '3.14' 가 3으로 형변환 된다.
    console.log('d = ', d, typeof d);

    d = parseFloat('300'); // '300' 이 300으로 형변환 된다.
    console.log('d = ', d, typeof d);
