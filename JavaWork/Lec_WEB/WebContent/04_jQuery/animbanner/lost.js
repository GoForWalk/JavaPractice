(function(){
    ...
    (function loopSea(){
        ...
        $.when($waterBack, $waterFront).done(loopSea);
    })();
    ...
    $boat.css({'left' : '-220px'}); // 최초, 화면 왼쪽 밖 위치
    ...
    //     })
    //     .then(function(){
    //         return $form.animate({'left' : '0px'}, 500 , reload)
    //     })
    // })
    // ;
    ...
    $boat.animate({'left' : '20px'}, 2000).promise()
        ...
        .then(function(){
            return $title2.animate({'opactiy' : 1}, 1000).promise();
        })
        .then(function(){
            return $form.animate({'left' : '0px'}, 500 , reload).promise();
        })
    ;
    ...
    function reload(){ // 모든 애니메이션 끝나면 5초뒤 리로드
        setTimeout(function(){
            location.reload();
        }, 5000);
    }
