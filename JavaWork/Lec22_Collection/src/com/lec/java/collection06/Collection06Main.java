...
/* Set, HashSet<E>
	...
	Collection<E>
	 |__ Set<E>
	      |__ HashSet<E>, TreeSet<E>  // Hash~ : 특정 데이터를 뽑아내는데 빠르다 ( 검색속도 특화)
...
public class Collection06Main {
	...
	public static void main(String[] args) {
		...
		set.add(1);
		set.add(2);
		System.out.println(set.add(100)); //false: 데이터 중복
		
		// HashSet의 크기: size()
		System.out.println("HashSet의 크키: " +  set.size());
		
		// Set은 중복 저장을 허용하지 않는다.
		
		// 데이터 검색:
		// Set은 인덱스가 없기 때문에 get() 메소드를 제공하지 않습니다. // 순서가 없기 때문에 자료 지목 X
...
//		set.remove(2); // 데이터 2를 삭제!!! (2번째 데이터 삭제가 아니다!!!)
		System.out.println("삭제 결과: " + set.remove(2)); //   element 있다면 삭제 후 true를 리턴
		System.out.println("삭제 결과: " + set.remove(2));
		itr = set.iterator(); // Iterator는 일회용이라 다시 선언
		
		System.out.println("데이터 삭제: set.remove(2)");
		while(itr.hasNext()) {
			System.out.println(itr.next());
			
		}
//		itr.next(); //err: java.util.NoSuchElementException // Iterator는 일회용이라 다시 선언
