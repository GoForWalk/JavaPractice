...
//		LinkedList<String> list1 = new LinkedList<String>();
		...
		System.out.println("sort()");  //List의 toString() 결과 형태 출력
		// sort()
		...
		// "2018-08-06" --> Date 객체
		list2.add(new SimpleDateFormat("yyyy-MM-dd").parse("2018-08-06"));
		...
		list3.add(new Student("Kevin", 90));
		System.out.println(list3);
		Collections.sort(list3); // sort 안되는 이유 err : Student에  Comparable<T> 인터페이스가 구현 안되어 있다.
		
		
		
		// Comparable<> 구현 후 실행						
		//compareTo overriding 하여 오름차순, 내림차순 설정할 수 있다.
		...
		arrList.subList(0, 3);  //index 0 부터 3 전까지의 List<> 생성
		...
		System.out.println(Collections.min(list3)); // Comparable이 설정 되있어야만 동작한다.
		...
		Collections.copy(list4, list3); // list 4에 list 3 가 덮어씌워 진다.
		System.out.println(list4);
		System.out.println(list3);
...
// Student 를 이름(name)으로 오름차순으로 정렬해주는 Comparator<>를 만든다.
...
// Student 를 이름(name)으로 내림차순으로 정렬해주는 Comparator<>를 만든다.
